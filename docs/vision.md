...
- Dead code 清理（第七轮）：移除 section_merge.py 整个文件（169 行）+ vector_store.count（5 行）+ content_schema.collect_changed_sections（10 行）+ orchestrator 4 个未使用导入（wait、FIRST_COMPLETED、re、Entity），共净减约 188 行
- Dead code 清理（第八轮）：移除 api.py _TEXT_EXTENSIONS + _read_file_content + send_from_directory 导入；两后端 register_entity_redirects / update_relation_episode_id / entity_has_any_relation / delete_orphan_entities / get_self_referential_relations / delete_self_referential_relations / get_self_referential_relations_for_entity / delete_self_referential_relations_for_entity / get_entity_degree（共 18 个方法）+ graph_traversal.get_entity_degree + manager.py import os + test_self_referential_relations_detection 测试，共净减约 450 行
- Dead code 清理（第九轮）：移除 orchestrator start_scheduler/stop_scheduler/history（56 行）+ task_queue _entity_col_progress/_relation_col_progress（10 行）+ monitor get_graph_ids（3 行）+ sse make_queue_callback（11 行）+ visualizer.py 整个文件（343 行）+ web.py visualizer 导入和实例化清理 + content_merger merge_entity_section/merge_relation_section（93 行）+ prompts MERGE_ENTITY_SECTION_SYSTEM_PROMPT/MERGE_RELATION_SECTION_SYSTEM_PROMPT（22 行）+ neo4j_store db_path 属性（4 行），共净减约 540 行
- Dead code 清理（第十轮）：移除 entity.py 未使用导入（clean_markdown_code_blocks, clean_separator_tags, render_markdown_sections）+ relation.py 整块未使用 content_schema 导入（RELATION_SECTIONS, content_to_sections, compute_section_diff, has_any_change, section_hash）+ debug_log.py init() 函数及 os 导入，共净减约 22 行
- 静默异常修复（第一轮）：api.py search/stream 端点 embedding 失败和 synthesize_answer 失败添加日志 + chat_session.py 进程终止失败添加日志 + neo4j_store.py provenance JSON 解析失败添加日志 + manager.py 连接健康检查失败添加日志 + vector_store.py 连接关闭失败添加日志 + web.py 版本计数获取失败添加日志 + dashboard.py Live 停止失败添加日志 + registry.py 队列/存储关闭失败添加日志 + chat_api.py OpenAI 客户端关闭失败添加日志 + task_queue.py 3 处 JSON 解析/日志写入失败添加日志（共 15 处 `except Exception: pass` → 带日志记录）
- 性能优化：manager.py get_latest_episode 避免循环中重复加载完整 episode（只保留最新 cache_id，最后加载一次）+ neo4j_store.py save_content_patches N 次逐条 CREATE → UNWIND 单次批量写入
- Dream N+1 查询消除：orchestrator _discover_relations 批量预取所有配对的已有关系（get_relations_by_entity_pairs），避免 _judge_pair 中 O(N) 次 get_relations_by_entities 逐对查询
- 静默异常修复（续）：api.py 关闭信号处理和 atexit 清理（4 处）+ 端口排查提示（1 处）+ chat_api.py _as_dict 转换失败（1 处），补齐上一轮遗漏的 6 处
- Neo4j 批量 resolve_family_ids 优化：batch_get_entity_profiles 和 get_family_ids_by_names 中 N 次逐条 resolve_family_id → 1 次 resolve_family_ids 批量调用
- web.py 版本计数全面批量化：3 处 get_entity_version_count 逐条查询 → 批量 get_entity_version_counts 预取（节点构建、搜索图、关联实体节点），消除 O(N) 逐条查询
- graph_traversal.py BFS 种子 resolve 批量化：N 次 resolve_family_id → 1 次 resolve_family_ids（含回退）
- web.py 关联实体批量预取：循环中 get_entity_by_absolute_id → 批量 get_entities_by_absolute_ids 预取
- entity.py 实体名到ID映射批量存在性检查：N次 get_entity_by_family_id → 1次 resolve_family_ids 批量预判
- extraction.py Step7 关系写入前 resolve + 存在性检查批量化：N次 resolve_family_id + N次 get_entity_by_family_id → 各1次批量调用
- graph_traversal.py hasattr 兜底清理：get_entities_by_absolute_ids 双后端均已实现，移除 getattr + fallback 循环
- 未使用 import 清理：agent_query json、consolidation json、extraction hashlib/threading、relation ContentPatch、content_schema Optional、monitor Any（共 7 处）
- web.py O(N)列表推导查重优化：[node['id'] for node in nodes] → set 预计算，循环内 O(N) → O(1)
