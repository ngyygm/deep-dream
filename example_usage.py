...
if __name__ == "__main__":
    ...
    processor = TemporalMemoryGraphProcessor(
        ...
        # llm_model="glm-4.6",  # LLM模型名称（可选）
        # llm_base_url="https://open.bigmodel.cn/api/coding/paas/v4",  # LLM API基础URL（可选）
        llm_api_key="b0d9820b40ea427cb1df20af36fe2fc7.hv3LrdurAFxiXDGV",  # LLM API密钥（可选）
        ...
        embedding_device="cuda:1",  # Embedding计算设备 ("cpu" 或 "cuda")
        similarity_threshold=0.7,  # 实体搜索相似度阈值
        max_similar_entities=10,  # 语义向量初筛后返回的最大相似实体数量（默认10）
        content_snippet_length=64,  # 用于相似度搜索的实体content截取长度（默认50字符）
        relation_content_snippet_length=256,  # 用于embedding计算的关系content截取长度（默认50字符）
        # 实体抽取配置
        entity_extraction_max_iterations=3,  # 实体抽取最大迭代次数（默认3次）
        relation_extraction_absolute_max_iterations=10,  # 关系抽取绝对最大迭代次数（默认10次），超过后强制停止，防止无限循环
        ...
        # 关系抽取配置
        relation_extraction_max_iterations=5,  # 关系抽取最大迭代次数（默认3次）
