...
- LongMemEval：Di Wu；Hongwei Wang；Wenhui Yu；Yuwei Zhang；Kai-Wei Chang；Dong Yu。
...
实际命令包括 `deepxiv search 'LoCoMo Plus' --limit 3 --format json`、`deepxiv paper  --brief`（8 条原引用＋LoCoMo-Plus），以及 Semantic Scholar `/graph/v1/paper/ARXIV:?fields=title,authors.name,year,venue,publicationVenue,publicationDate,externalIds,url`。DeepXiv TLDR 仅辅助发现，最终身份采用原始 metadata；不以 AI 摘要当原文。
...
| `published-baselines.md` | `c675104a-37b5-495c-80e6-df5a7584f317` |
...
*来源：https://github.com/ngyygm/deep-dream/blob/main/paper-audit.md（远程 main，2026-09-21 审查，审查对象为当时 13 页快照）；2026-09-22 归档至本仓 `research/reports/`。Phase A 已按本报告修复其中论文层面的条目（单段摘要、跨协议主张撤回、重加权标注、列反置、std 指针、LoCoMo-Plus 引用）。*
