...
{\footnotesize\textbf{（a）MemoryAgentBench（官方评分器）}}\\[1.5pt]
...
\toprule
记忆系统 & 答题模型 & AR & TTL & LRU & SF & Overall \\
\midrule
...
\rowcolor{blue!9}
完整上下文（$n{=}614$） & Kimi-K3 & \textbf{96.8} & \textbf{75.0} & 65.2 & 78.0 & \textbf{78.7} \\
\midrule
...
\rowcolor{blue!9}
\textbf{Deep-Dream / 原文读取} & \textbf{Kimi-K3} & 88.8 & 52.9 & 62.3 & \textbf{80.0} & 71.0 \\
\bottomrule
\end{tabular*}

\vspace{3pt}
{\footnotesize\textbf{（b）LoCoMo 二值 $J$（\%；Mem0 协议）}}\\[1.5pt]
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.15\textwidth}rrrrr@{}}
\toprule
记忆系统 & 答题模型 / 评审模型 & 单跳 & 多跳 & 开放域 & 时序 & 加权 \\
\midrule
...
\toprule
记忆系统 & \shortstack[l]{答题模型 /\\ 评审模型} & Overall & 会话偏好 & 会话助手 & 时序 & 多会话 & 知识更新 & 会话用户 \\
\midrule
...
\midrule
\rowcolor{blue!9}
\textbf{Deep-Dream} & \shortstack[l]{\textbf{Qwen3.7-plus /}\\\textbf{Qwen3.7-plus}} & 84.6 & 53.3 & \textbf{100.0} & 84.2 & 74.4 & 91.0 & 98.6 \\
\bottomrule
\end{tabular*}}
\caption{Deep-Dream 与公开记忆系统在 MemoryAgentBench、LoCoMo 和 LongMemEval-S 上的结果。每一行保留答题模型、评审模型、测试子集和评测设置。Deep-Dream 的 LoCoMo 行是单次诊断运行，旧指纹说明见附录。加粗标记每列在所显示各行中的最大值；Infini Memory-A 行使用其论文报告的类别平均值。}
