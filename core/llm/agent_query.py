...
from .sanitize import sanitize_user_input, wrap_user_content, check_for_prompt_leaks
...
class AgentQueryMixin:
    ...
    async def agent_meta_query(self, question: str, graph_id: str = "default") -> Dict[str, Any]:
        ...
        safe_question, was_modified = sanitize_user_input(question, max_length=5000)
        if was_modified:
            logger.warning("User question was sanitized in agent_meta_query")

        # 步骤 1：解析查询意图
        intent = await self._parse_query_intent(safe_question)

        # 步骤 2：执行查询
        results = await self._execute_query(intent, graph_id)

        # 步骤 3：生成自然语言回答
        answer = await self._generate_answer(safe_question, results)
...
请分析图谱并给出改进建议："""
    ...
    async def _parse_query_intent(self, question: str) -> Dict[str, Any]:
        ...
        safe_question, was_modified = sanitize_user_input(question, max_length=5000)
        if was_modified:
            logger.warning("User question was sanitized in _parse_query_intent")

        # Wrap user content with safe delimiters
        safe_content = wrap_user_content(safe_question)
...
请分析问题并返回查询意图："""
        ...
        try:
            ...
            # Check for prompt leaks in response
            if isinstance(result, dict) and "answer" in result:
                if check_for_prompt_leaks(str(result.get("answer"))):
                    logger.warning("Potential prompt leak detected in query intent response")
                    return {"query_type": "hybrid", "query_text": safe_question}
            return result if isinstance(result, dict) else {"query_type": "hybrid", "query_text": safe_question}
        except Exception as e:
    ...
    def synthesize_answer(self, question: str, entity_dicts: list, relation_dicts: list) -> str:
        ...
        safe_question, was_modified = sanitize_user_input(question, max_length=5000)
        if was_modified:
            logger.warning("User question was sanitized in synthesize_answer")
...
请基于以上知识图谱检索结果，简洁地回答用户的问题。如果检索结果不足以完整回答，请基于已有信息给出部分回答并指出信息缺口。直接输出回答文本，不要使用 JSON 格式。"""
        ...
        try:
            result = self._call_llm(prompt="", messages=messages)
            # Check for prompt leaks in response
            if result and check_for_prompt_leaks(result):
                logger.warning("Potential prompt leak detected in synthesize_answer response")
                return "无法基于检索结果生成回答。"
