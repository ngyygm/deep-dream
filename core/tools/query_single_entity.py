...
def print_relations(relations, storage, show_all=False):
    ...
    # Batch-fetch all related entities at once (avoid N+1)
    other_abs_ids = list({rel.entity2_absolute_id for rel in relations if rel.entity2_absolute_id})
    other_entities_map = {}
    if other_abs_ids:
...
def print_summary(result, storage):
    ...
    # 关系统计 — batch-fetch instead of N+1
    if relations:
        other_abs_ids = list({rel.entity2_absolute_id for rel in relations if rel.entity2_absolute_id})
        related_names = set()
        if other_abs_ids:
            other_entities = storage.get_entities_by_absolute_ids(other_abs_ids)
            related_names = {e.name for e in other_entities if e}

        print(f"关联实体数: {len(related_names)}")
        print(f"关联实体: {', '.join(list(related_names)[:10])}")
