...
class QuestionAnalyzer:
    ...
    # Pre-compiled classification patterns
    _ENTITY_RELATION_PATTERNS = [
        re.compile(r'(.{1,5})和(.{1,5})的?(关系|互动|对话|交流)'),
        re.compile(r'(.{1,5})与(.{1,5})的?(关系|互动|对话|交流)'),
        re.compile(r'(.{1,5})和(.{1,5})'),
    ]
    _SINGLE_ENTITY_PATTERNS = [
        re.compile(r'(.{1,10})是谁'),
        re.compile(r'(.{1,10})是什么'),
        re.compile(r'告诉我关于(.{1,10})'),
        re.compile(r'描述(.{1,10})'),
    ]
    _NETWORK_PATTERNS = [
        re.compile(r'关系网络'),
        re.compile(r'(展示|显示).{0,5}关系'),
        re.compile(r'和谁有联系'),
        re.compile(r'关联图'),
    ]
    _TIMELINE_PATTERNS = [
        re.compile(r'(时间|变化|发展|演变)'),
        re.compile(r'(开始|最初|最初时)'),
        re.compile(r'(过程|经历)'),
        re.compile(r'时间线'),
    ]
    _SEARCH_PATTERNS = [
        re.compile(r'(搜索|查找|找)'),
        re.compile(r'(哪些|哪些地方|哪里)'),
        re.compile(r'(提到|关于)'),
    ]
    _RECONSTRUCTION_PATTERNS = [
        re.compile(r'(如何|怎么)(发现|知道|了解)'),
        re.compile(r'(经过|过程|来龙去脉)'),
        re.compile(r'(重建|还原|描述)(事件|过程)'),
    ]
    _CJK_NAME_RE = re.compile(r'[\u4e00-\u9fff]{2,10}')
    ...
    def _is_entity_relation_question(question):
        """是否是实体关系问题"""
        return any(p.search(question) for p in QuestionAnalyzer._ENTITY_RELATION_PATTERNS)

    @staticmethod
    def _is_single_entity_question(question):
        """是否是单实体问题"""
        return any(p.search(question) for p in QuestionAnalyzer._SINGLE_ENTITY_PATTERNS)

    @staticmethod
    def _is_network_question(question):
        """是否是关系网络问题"""
        return any(p.search(question) for p in QuestionAnalyzer._NETWORK_PATTERNS)

    @staticmethod
    def _is_timeline_question(question):
        """是否是时间线问题"""
        return any(p.search(question) for p in QuestionAnalyzer._TIMELINE_PATTERNS)

    @staticmethod
    def _is_search_question(question):
        """是否是搜索问题"""
        return any(p.search(question) for p in QuestionAnalyzer._SEARCH_PATTERNS)

    @staticmethod
    def _is_reconstruction_question(question):
        """是否是事件重建问题"""
        return any(p.search(question) for p in QuestionAnalyzer._RECONSTRUCTION_PATTERNS)
...
class QueryOrchestrator:
    ...
    def _handle_reconstruction(self, question, auto_expand):
        ...
        # Batch-load episodes instead of N individual calls
        _batch_fn = getattr(self.storage, 'load_episodes', None)
        if _batch_fn and cache_files:
            _ids = [f.stem for f in cache_files[:20]]
            _all_episodes = _batch_fn(_ids)
            _kw_list = [kw.lower() for kw in keywords.split()]
            for cache in _all_episodes:
                if cache:
                    _content = (cache.content or "").lower()
                    if any(kw in _content for kw in _kw_list):
                        matched_caches.append(cache)
        else:
            for cache_file in cache_files[:20]:
                try:
                    cache = self.storage.load_episode(cache_file.stem)
                    if cache and any(kw in (cache.content or "").lower() for kw in _kw_list):
                        matched_caches.append(cache)
                except Exception:
                    continue
    ...
    def _extract_entity_names(self, text):
        """提取实体名称（简单实现）"""
        matches = QuestionAnalyzer._CJK_NAME_RE.findall(text)
