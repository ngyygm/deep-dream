...
class GraphMonitor:
    ...
    def snapshot(self) -> dict:
        ...
        # Single get_monitor_snapshot call shared by queue + thread stats
    ...
    def _collect_queue_stats(self, _snap: dict = None) -> dict:
        try:
            snapshot = _snap or self._queue.get_monitor_snapshot(limit=10)
            return {
    ...
    def _collect_thread_stats(self, _queue_snap: dict = None) -> dict:
        try:
            ...
            qs = _queue_snap or self._queue.get_monitor_snapshot(limit=0)
            return {
                "python_threads_total": len(threads),
                "remember_worker_threads_alive": remember_alive,
                "remember_worker_threads_busy": qs["running_count"],
