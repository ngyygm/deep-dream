...
_VALID_STRATEGIES = frozenset(("random", "orphan", "hub", "time_gap", "cross_community", "low_confidence"))
...
def butler_execute():
    ...
    try:
        ...
        def _run_action(action):
            """Run a single independent butler action."""
        ...
        if len(independent) > 1:
            futures = {_dream_pool.submit(_run_action, a): a for a in independent}
            for f in futures:
                action = futures[f]
                try:
            ...
            for action in independent:
                results[action] = _run_action(action)
