"""
Helper functions extracted from _search.py — body logic for the largest route
handlers, so that _search.py stays under 800 lines while keeping all
@relations_bp.route decorators there.
"""
...
# Re-exported from parent package so helpers can use them without circular imports
from core.server.blueprints.relations import (
    relations_bp,        # noqa: F401 — not used directly but keeps import consistent
    _shared_pool,
    _PAREN_ANNOTATION_RE,
)
...
# find_unified  body
# -----------------------------------------------------------------------
def find_unified_body() -> tuple:
    """Execute the unified semantic retrieval pipeline.

    Returns ``(result_dict, status_code)`` on success, or raises so the
    thin wrapper can catch and return an error response.
    """
    body = get_json_body()
...
# -----------------------------------------------------------------------
# find_relations_search body
# -----------------------------------------------------------------------
def find_relations_search_body() -> tuple:
...
# -----------------------------------------------------------------------
# find_relations_by_entity body
# -----------------------------------------------------------------------
_VALID_RELATION_SCOPES = frozenset(("accumulated", "version_only", "all_versions"))


def find_relations_by_entity_body(family_id: str):
    processor = _get_processor()
...
# -----------------------------------------------------------------------
# create_relation body
# -----------------------------------------------------------------------
def create_relation_body():
