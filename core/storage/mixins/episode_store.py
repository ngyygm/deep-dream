...
# TTL cache for frequently-polled episode metadata queries
_episode_cache: dict = {}  # key -> (timestamp, value)
_episode_cache_ttl = 5.0  # seconds
...
class EpisodeStoreMixin:
    ...
    def _iter_cache_meta_files(self) -> List[Path]:
        """迭代所有 cache 元数据文件（带短 TTL 缓存避免重复 glob+sort）"""
        now = time.monotonic()
    ...
    def get_latest_episode(self, activity_type: Optional[str] = None) -> Optional[Episode]:
        ...
        try:
            ...
            if activity_type:
                cursor.execute(
                    "SELECT id FROM episodes WHERE activity_type = ? "
                    "ORDER BY event_time DESC LIMIT 1",
                    (activity_type,)
                )
            else:
                cursor.execute(
                    "SELECT id FROM episodes ORDER BY event_time DESC LIMIT 1"
                )
            row = cursor.fetchone()
            if row:
                return self.load_episode(row[0])
        except Exception as exc:
            logger.debug("SQLite latest episode query failed: %s", exc)

        # Fallback: file scan (legacy)
        ...
        if latest_cache_id:
            return self.load_episode(latest_cache_id)
        return None
    ...
    def get_latest_episode_metadata(self, activity_type: Optional[str] = None) -> Optional[Dict]:
        ...
        Returns:
            ...
            - doc_hash: 文档 hash
        """
        # Fast path: query SQLite to find the latest episode's doc_hash
        try:
            conn = self._get_conn()
            cursor = conn.cursor()
            if activity_type:
                cursor.execute(
                    "SELECT id, doc_hash FROM episodes WHERE activity_type = ? "
                    "ORDER BY event_time DESC LIMIT 1",
                    (activity_type,)
                )
            else:
                cursor.execute(
                    "SELECT id, doc_hash FROM episodes ORDER BY event_time DESC LIMIT 1"
                )
            row = cursor.fetchone()
            if row:
                cache_id, doc_hash = row[0], row[1]
                # Load metadata from file using doc_hash
                if doc_hash:
                    meta_path = self.docs_dir / doc_hash / "meta.json"
                    if meta_path.exists():
                        with open(meta_path, "r", encoding="utf-8") as f:
                            return json.load(f)
            ...
            logger.debug("SQLite latest episode metadata query failed: %s", exc)
    ...
    def search_episodes_by_bm25(self, query: str, limit: int = 20) -> List[Episode]:
        ...
        if cursor.fetchone()[0] > 0:
            ...
            # Construct Episodes directly from SQL rows (skip redundant load_episode disk reads)
            results = []
    ...
    def list_docs(self) -> List[Dict[str, Any]]:
        ...
        优先使用 SQLite episodes 表查询（O(D) where D = distinct documents），
        回退到文件遍历。
        """
        # Fast path: query SQLite for distinct documents
        try:
            conn = self._get_conn()
            cursor = conn.cursor()
            cursor.execute("""
                SELECT source_document, doc_hash, activity_type,
            ...
            for row in cursor.fetchall():
                ...
                event_time_str = row[3] or None
                processed_time_str = row[4] or None
                episode_count = row[5]

                # Try to get text length from file
                text_len = 0
                if doc_hash:
                    doc_dir = self.docs_dir / doc_hash
                    original_path = doc_dir / "original.txt"
            ...
            logger.debug("list_docs SQLite query failed: %s", exc)
    ...
    def _save_episode_to_db(self, cache: Episode, doc_hash: str = ""):
        ...
        try:
            conn = self._get_conn()
            # Pre-compute concept embedding OUTSIDE lock (ML inference is slow)
            ...
            with self._write_lock:
                ...
                """, (
                    ...
                    (cache.processed_time.isoformat() if cache.processed_time else _now_iso),
                ...
                self._write_concept_from_episode(cache, doc_hash, cursor,
                                                  ...
                                                  precomputed_embedding=_concept_emb,
                                                  _now_iso=_now_iso)
                conn.commit()
        except Exception as exc:
    ...
    def get_episode_from_db(self, episode_id: str) -> Optional[Dict]:
        ...
        _cache_key = f"ep:{episode_id}"
        _cached = _episode_cache.get(_cache_key)
        _now = time.time()
        if _cached is not None and (_now - _cached[0]) < _episode_cache_ttl:
            return _cached[1]
        conn = self._get_conn()
        cursor = conn.cursor()
