...
class DreamStoreMixin:
    ...
    def _dream_seeds_orphan(self, count: int, exclude_ids: list) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            for e in isolated
            if e.family_id not in _exclude
        ]
    ...
    def _dream_seeds_hub(self, count: int, exclude_ids: list) -> List[Dict[str, Any]]:
        ...
        _exclude = set(exclude_ids)
        fids = [row[0] for row in rows if row[0] not in _exclude]
        # Batch fetch entities
        if fids:
            entity_map = self.get_entities_by_family_ids(fids)
        else:
            entity_map = {}
        for row in rows:
            fid = row[0]
            ...
            if entity:
                seeds.append({
                    "family_id": entity.family_id,
                    "name": entity.name,
                    "content": entity.content[:200] if entity.content else "",
                    "confidence": entity.confidence,
                    "event_time": entity.event_time.isoformat() if entity.event_time else None,
                    "degree": degree,
                })
    ...
    def _dream_seeds_time_gap(self, count: int, exclude_ids: list) -> List[Dict[str, Any]]:
        ...
        for row in cursor.fetchall():
            entity = self._row_to_entity(row)
            if entity.family_id in _exclude:
                continue
            seeds.append({
                "family_id": entity.family_id,
                "name": entity.name,
    ...
    def _dream_seeds_cross_community(self, count: int, exclude_ids: list) -> List[Dict[str, Any]]:
        ...
        # Step 1: 获取所有 entity 行 + 建立 aid_to_fid 映射（单次查询替代两次全表扫描）
        cursor.execute("SELECT id, family_id FROM entities WHERE invalid_at IS NULL")
        aid_to_fid: Dict[str, str] = {}
        all_fids_set: set = set()
        for row in cursor.fetchall():
            aid_to_fid[row[0]] = row[1]
            all_fids_set.add(row[1])
        all_fids = list(all_fids_set)
        if len(all_fids) < 2:
        ...
        # Step 3: 通过 relations 表的 absolute_id 映射连接分量
        cursor.execute("""
        ...
        # Step 5: Pre-fetch all entities from valid components, then random pair
        all_component_fids = list({fid for comp in valid_components for fid in comp})
        entity_cache = {}
        if all_component_fids:
            entity_cache = self.get_entities_by_family_ids(all_component_fids)
        pairs = []
        attempts = 0
        max_attempts = count * 3
        while len(pairs) < count and attempts < max_attempts:
            c1, c2 = random.sample(valid_components, 2)
            fid1 = random.choice(c1)
            fid2 = random.choice(c2)
            entity1 = entity_cache.get(fid1)
            entity2 = entity_cache.get(fid2)
            if entity1 and entity2:
                pairs.append({
                    "family_id": entity1.family_id,
                    "name": entity1.name,
                    "content": entity1.content[:200] if entity1.content else "",
    ...
    def save_dream_relation(self, entity1_id: str, entity2_id: str,
        ...
        return self.save_dream_relation_fast(
            entity1, entity2, resolved1, resolved2,
            content, confidence, reasoning,
            dream_cycle_id=dream_cycle_id,
            episode_id=episode_id,
        )

    def save_dream_relation_fast(self, entity1: Any, entity2: Any,
                                 resolved1: str, resolved2: str,
                                 content: str, confidence: float, reasoning: str,
                                 dream_cycle_id: Optional[str] = None,
                                 episode_id: Optional[str] = None) -> Dict[str, Any]:
        """Same as save_dream_relation but skips entity resolution (caller pre-fetches)."""
    ...
    def reject_dream_cycle_relations(self, dream_cycle_id: str) -> Dict[str, Any]:
        ...
        """, [_dream_source(dream_cycle_id)])

        family_ids = [row[0] for row in cursor.fetchall()]
        if not family_ids:
            return {"rejected_count": 0, "cycle_id": dream_cycle_id}

        # Batch resolve + batch fetch relations
        resolved_map = self.resolve_family_ids(family_ids)
        resolved_fids = list({resolved_map.get(fid, fid) for fid in family_ids})

        # Batch fetch the latest version of each relation
        now = datetime.now()
        _reason = f"批量拒绝: dream cycle {dream_cycle_id}"
        _now_iso = now.isoformat()
        new_relations = []
        with self._write_lock:
            ph = ",".join("?" * len(resolved_fids))
            cursor.execute(f"""
                SELECT r.id, r.family_id, r.entity1_absolute_id, r.entity2_absolute_id,
                       r.content, r.event_time, r.processed_time, r.episode_id,
                       r.source_document, r.confidence, r.attributes
                FROM relations r
                WHERE r.family_id IN ({ph})
                  AND r.invalid_at IS NULL
                  AND json_extract(r.attributes, '$.tier') = 'candidate'
            """, resolved_fids)
            rows = cursor.fetchall()
        ...
        for row in rows:
            try:
                ...
                attrs = {}
            attrs["status"] = "rejected"
            attrs["rejected_reason"] = _reason
            attrs["rejected_at"] = _now_iso
            ...
            new_relations.append(Relation(
                ...
                confidence=min(row[9] or 0.3, 0.2),
                attributes=json.dumps(attrs),
            ))

        if new_relations:
            self.bulk_save_relations(new_relations)

        return {
            "rejected_count": len(new_relations),
    ...
    def save_dream_episode(self, content: str,
        ...
        # Record entity mentions — batch resolve + batch fetch
        if entities_examined:
            ...
            abs_ids = []
            if canonical_fids:
                entity_map = self.get_entities_by_family_ids(canonical_fids)
                for eid in entities_examined:
                    resolved = resolved_map.get(eid)
                    entity = entity_map.get(resolved) if resolved else None
                    if entity:
