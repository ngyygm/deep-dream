    ...
    def search_entities_by_bm25(self, query: str, limit: int = 20,
        ...
        try:
            with self._session() as session:
                ...
                logger.debug("BM25 search: query=%r limit=%d graph_id=%s", safe_query, limit, getattr(self, '_graph_id', '?'))
                ...
                for record in result:
                    ...
                    if len(raw_entities) >= limit:
                        break
                logger.debug("BM25 search done: query=%r raw_entities=%d graph_id=%s", safe_query, len(raw_entities), getattr(self, '_graph_id', '?'))
