...
def _cached_bigram_set(text: str) -> frozenset:
    """Cached bigram set computation for entity text similarity pre-filtering."""
    if len(text) < 2:
        return frozenset()
...
class StorageManager(EntityStoreMixin, RelationStoreMixin, EpisodeStoreMixin, ConceptStoreMixin, DreamStoreMixin):
    ...
    # Pre-computed aliased column strings — avoids split+join on every query call
    _ENTITY_SELECT_E = "e." + _ENTITY_SELECT.replace(", ", ", e.")
    _ENTITY_SELECT_E1 = "e1." + _ENTITY_SELECT.replace(", ", ", e1.")
    _RELATION_SELECT_R = "r." + _RELATION_SELECT.replace(", ", ", r.")
    _RELATION_SELECT_R1 = "r1." + _RELATION_SELECT.replace(", ", ", r1.")
    ...
    def __init__(self, storage_path: str, embedding_client=None,
        ...
        self._relation_emb_cache_all: Optional[List[tuple]] = None
        self._relation_emb_cache_all_ts: float = 0.0
        ...
        # TTL cache for resolve_family_ids — avoids repeated redirect lookups
        self._resolve_cache: dict = {}  # family_id -> (timestamp, canonical_id)
        self._resolve_cache_ts: float = 0.0
        self._resolve_cache_ttl: float = 5.0  # seconds
    ...
    def _get_conn(self):
        ...
        健康检查节流：每 5 秒最多执行一次 SELECT 1，减少 SQLite 开销。"""
        conn = getattr(self._local, 'conn', None)
        if conn is not None:
            # Throttle health checks: only check every 5 seconds
            _last_check = getattr(self._local, '_conn_last_check', 0.0)
            _now = time.monotonic()
            if _now - _last_check < 5.0:
                return conn
            try:
                conn.execute("SELECT 1 FROM entities LIMIT 0")
                self._local._conn_last_check = _now
                return conn
    ...
    def _vector_store_upsert_entity(self, absolute_id: str, embedding) -> None:
        ...
        try:
            if isinstance(embedding, bytes):
                vs.upsert("entity_vectors", absolute_id, [], _precomputed_bytes=embedding)
            else:
                vs.upsert("entity_vectors", absolute_id, embedding)
        except Exception as e:
    ...
    def _vector_store_backfill(self) -> int:
        ...
        Uses batch upsert to avoid per-row commits.
        """
        ...
        try:
            conn = self._get_conn()
            cursor = conn.cursor()
            # Backfill entity embeddings (batched)
            ...
            for abs_id, emb_blob in cursor.fetchall():
                if emb_blob:
                    batch.append((abs_id, emb_blob))
                    if len(batch) >= batch_size:
                        vs.upsert_batch("entity_vectors", batch)
                        count += len(batch)
                        batch = []
            if batch:
                vs.upsert_batch("entity_vectors", batch)
                count += len(batch)
            # Backfill relation embeddings (batched)
            ...
            for abs_id, emb_blob in cursor.fetchall():
                if emb_blob:
                    batch.append((abs_id, emb_blob))
                    if len(batch) >= batch_size:
                        vs.upsert_batch("relation_vectors", batch)
                        count += len(batch)
                        batch = []
            if batch:
                vs.upsert_batch("relation_vectors", batch)
    ...
    def _vector_store_upsert_relation(self, absolute_id: str, embedding) -> None:
        ...
        try:
            if isinstance(embedding, bytes):
                vs.upsert("relation_vectors", absolute_id, [], _precomputed_bytes=embedding)
            else:
                vs.upsert("relation_vectors", absolute_id, embedding)
        except Exception as e:
    ...
    def save_content_patches(self, patches: list):
        ...
        if not patches:
            return
        conn = self._get_conn()
        # Pre-build rows OUTSIDE lock (pure computation)
        ...
        rows = [
            (p.uuid, p.target_type, p.target_absolute_id, p.target_family_id,
             ...
             p.event_time.isoformat() if p.event_time else _now)
            for p in patches
        ]
    ...
    def resolve_family_ids(self, family_ids: List[str]) -> Dict[str, str]:
        ...
        Results are TTL-cached for 5 seconds to avoid repeated redirect lookups
        across multiple calls in the same pipeline pass.
        ...
        if not family_ids:
            return {}
        unique_ids = list({fid.strip() for fid in family_ids if fid and fid.strip()})
        if not unique_ids:
            return {}

        # TTL cache check — serve from cache if fresh
        now = time.time()
        if self._resolve_cache and (now - self._resolve_cache_ts) < self._resolve_cache_ttl:
            cached = {}
            uncached_ids = []
            for fid in unique_ids:
                entry = self._resolve_cache.get(fid)
                if entry is not None:
                    cached[fid] = entry
                else:
                    uncached_ids.append(fid)
            if not uncached_ids:
                # All from cache — build output
                return {fid: cached.get(fid.strip(), fid) for fid in family_ids}
            # Partially cached — only query uncached IDs
            unique_ids = uncached_ids
        else:
        ...
        # Merge DB results with cached results, then update cache
        merged = {**cached, **result}
        now = time.time()
        for fid, canonical in result.items():
            self._resolve_cache[fid] = canonical
        self._resolve_cache_ts = now

        output: Dict[str, str] = {}
        for fid in family_ids:
            key = fid.strip() if fid else ""
            output[fid] = merged.get(key, key)
    ...
    def _invalidate_emb_cache(self):
        ...
        with self._emb_cache_lock:
            ...
            self._relation_emb_cache_all = None
            self._relation_emb_cache_all_ts = 0.0
            self._concept_emb_cache = None
            self._concept_emb_cache_ts = 0.0
            # Clear resolve cache too — redirects may have changed
            self._resolve_cache = {}
            self._resolve_cache_ts = 0.0
    ...
    def _search_with_embedding(self, query_text: str, entities_with_embeddings: List[tuple],
        ...
        if entities_to_encode:
            if text_mode == "name_only":
                entity_texts = [e.name for e in entities_to_encode]
            elif text_mode == "content_only":
                entity_texts = [e.content for e in entities_to_encode]
            elif use_content:
                entity_texts = [f"{e.name} {e.content}" for e in entities_to_encode]
            else:
                entity_texts = [e.name for e in entities_to_encode]
        ...
        _emb_matrix = np.array([emb for _, emb in stored_embeddings], dtype=np.float32)  # (N, D)
        _indices = [idx for idx, _ in stored_embeddings]
        _norms = np.linalg.norm(_emb_matrix, axis=1, keepdims=True)
        _norms = np.maximum(_norms, 1e-9)
        _emb_normed = _emb_matrix / _norms
        _query_norm = np.linalg.norm(query_embedding_array)
        if _query_norm > 0:
            _query_normed = query_embedding_array / _query_norm
        else:
            _query_normed = query_embedding_array
        _sims = _emb_normed @ _query_normed  # (N,)

        similarities = []
        for i, idx in enumerate(_indices):
            entity = entities_with_embeddings[idx][0]
            similarities.append((entity, float(_sims[i])))
    ...
    @staticmethod
    def _char_ngram_set(text: str, n: int) -> set:
    ...
    def _calculate_bleu_similarity(self, text1: str, text2: str) -> float:
        ...
        t1 = text1.lower()
        t2 = text2.lower()
        ng1_1 = self._char_ngram_set(t1, 1)
        ng2_1 = self._char_ngram_set(t2, 1)

        precision_1 = len(ng1_1 & ng2_1) / max(len(ng1_1), 1)
        if precision_1 == 0:
            return 0.0

        ng1_2 = self._char_ngram_set(t1, 2)
        ng2_2 = self._char_ngram_set(t2, 2)
        precision_2 = len(ng1_2 & ng2_2) / max(len(ng1_2), 1)
        if precision_2 == 0:
            return 0.0
    ...
    def _search_with_text_similarity(self, query_text: str, all_entities: List[Entity],
        ...
        _query_bigrams = _cached_bigram_set(_query_lower) if similarity_method == "jaccard" and len(_query_lower) >= 2 else None
        # Pre-compute query bigrams for text method's Jaccard pre-filter (invariant across loop)
        _text_query_bigrams = _cached_bigram_set(_query_lower) if similarity_method == "text" and len(_query_lower) >= 2 else None
        _ql = len(_query_lower)
        # Hoist text_mode branch outside the loop
        if text_mode == "name_only":
            _get_text = lambda e: e.name
        elif text_mode == "content_only":
            _get_text = lambda e: e.content
        elif use_content:
            _get_text = lambda e: f"{e.name} {e.content}"
        else:
            _get_text = lambda e: e.name

        scored_entities = []
        for entity in all_entities:
            entity_text = _get_text(entity)

            # 根据similarity_method计算相似度
            if similarity_method == "jaccard":
                if _query_bigrams is not None:
                    _et_lower_j = entity_text.lower().strip()
                    if _et_lower_j:
                        _eb = _cached_bigram_set(_et_lower_j)
                        _union = _query_bigrams | _eb
                        similarity = len(_query_bigrams & _eb) / len(_union) if _union else 0.0
                    else:
                        similarity = 0.0
                else:
            ...
            else:  # text (SequenceMatcher with Jaccard pre-filter)
                _et_lower = entity_text.lower()
                _el = len(_et_lower)
                if _ql > 0 and _el > 0 and abs(_ql - _el) > max(_ql, _el) * 0.6:
                    continue
                # Cached Jaccard bigram pre-filter (query bigrams pre-computed above)
                if _text_query_bigrams is not None and _el >= 2:
                    _ebigrams = _cached_bigram_set(_et_lower)
                    _jaccard = len(_text_query_bigrams & _ebigrams) / len(_text_query_bigrams | _ebigrams)
                    if _jaccard < threshold * 0.5:
                        continue
                similarity = difflib.SequenceMatcher(
                    None, _query_lower, _et_lower
                ).ratio()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"entities": entity_count, "relations": relation_count, "episodes": episode_count}
        except Exception:
    ...
    def get_graph_statistics(self) -> Dict[str, Any]:
        ...
        # Combined count query — single round-trip
        cursor.execute("""
            SELECT
                (SELECT COUNT(*) FROM entities),
                (SELECT COUNT(*) FROM relations),
                (SELECT COUNT(DISTINCT family_id) FROM entities),
                (SELECT COUNT(DISTINCT family_id) FROM relations)
        """)
        row = cursor.fetchone()
        entity_count = row[0]
        relation_count = row[1]
        unique_entities = row[2]
        unique_relations = row[3]
        ...
        if entity_count > 0:
            # Combined avg + max relation degree in one query
            cursor.execute("""
            ...
            """)
            deg_row = cursor.fetchone()
            stats["avg_relations_per_entity"] = round(deg_row[0], 2) if deg_row and deg_row[0] else 0
            stats["max_relations_per_entity"] = deg_row[1] if deg_row and deg_row[1] else 0
        ...
        # Combined time trends — single query with UNION ALL
        cursor.execute("""
            SELECT 'entity' as src, DATE(event_time) as d, COUNT(*) as cnt
            FROM entities GROUP BY d
            UNION ALL
            SELECT 'relation' as src, DATE(event_time) as d, COUNT(*) as cnt
            FROM relations GROUP BY d
            ORDER BY d
        """)
        entity_trends = []
        relation_trends = []
        for r in cursor.fetchall():
            entry = {"date": r[1], "count": r[2]}
            if r[0] == 'entity':
                entity_trends.append(entry)
            else:
                relation_trends.append(entry)
        stats["entity_count_over_time"] = entity_trends[:30]
        stats["relation_count_over_time"] = relation_trends[:30]
    ...
    def find_shortest_paths(self, source_family_id: str, target_family_id: str,
        ...
        for _depth in range(max_depth):
            ...
            for fid in queue:
                frontier_aids.extend(family_abs_ids.get(fid) or ())
            ...
            for row in frontier_rels:
                ...
                pair_key = (fid1, fid2) if fid1 <= fid2 else (fid2, fid1)
        ...
        for path_eids in all_paths_eid:
            for i in range(len(path_eids) - 1):
                _eid_a, _eid_b = path_eids[i], path_eids[i + 1]
                pair_key = (_eid_a, _eid_b) if _eid_a <= _eid_b else (_eid_b, _eid_a)
