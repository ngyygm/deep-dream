...
def _floats_to_bytes(vec: List[float], _cache={}) -> bytes:
...
def _bytes_to_floats(data: bytes) -> np.ndarray:
    """将 sqlite-vec 返回的 bytes 反序列化为 numpy float32 数组（零拷贝视图）。"""
...
class VectorStore:
    ...
    def _get_conn(self) -> sqlite3.Connection:
        ...
        跳过 SELECT 1 健康检查——SQLite WAL 模式下连接极少断开。
        首次验证通过 _local.healthy 标记，后续直接返回缓存连接。
        """
        conn = getattr(self._local, "conn", None)
        if conn is not None:
            # Only health-check on the first cached return per thread
            if not getattr(self._local, "healthy", False):
                try:
                    conn.execute("SELECT 1")
                    self._local.healthy = True
                except Exception:
                    conn.close()
                    self._local.conn = None
                    conn = None  # Fall through to create new connection
            if conn is not None:
                return conn
    ...
    def upsert(self, table: str, uuid: str, embedding, _precomputed_bytes=None) -> None:
        """插入或更新一条向量。
        ...
        Args:
            ...
            _precomputed_bytes: Pre-serialized bytes (skips _floats_to_bytes conversion).
        """
        conn = self._get_conn()
        emb_bytes = _precomputed_bytes or _floats_to_bytes(embedding)
    ...
    def upsert_batch(self, table: str, items) -> None:
        """批量插入或更新向量。

        items: List of (uuid, embedding) where embedding is either List[float] or pre-serialized bytes.
        """
        if not items:
            return
        conn = self._get_conn()
        ...
        rows = [(uuid, emb if isinstance(emb, bytes) else _floats_to_bytes(emb)) for uuid, emb in items]
        conn.executemany(
            f"INSERT INTO {table}(uuid, embedding) VALUES(?, ?)",
            rows,
        )
    ...
    def search(
        ...
        _precomputed_bytes: Optional[bytes] = None,
        ...
        Args:
            ...
            limit: 返回的最大条目数
            _precomputed_bytes: Optional pre-serialized query bytes (skips _floats_to_bytes)
        ...
        """
        conn = self._get_conn()
        query_bytes = _precomputed_bytes or _floats_to_bytes(query_embedding)
        cursor = conn.execute(
    ...
    def get(self, table: str, uuid: str) -> Optional[np.ndarray]:
        """获取指定 uuid 的向量。"""
        conn = self._get_conn()
        cursor = conn.execute(
    ...
    def get_batch(self, table: str, uuids: List[str]) -> Dict[str, np.ndarray]:
        """批量获取向量。"""
        if not uuids:
            return {}
        conn = self._get_conn()
