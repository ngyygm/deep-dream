...
def test_physical_graph_isolation_and_blob_scope(tmp_path):
    ...
    try:
        ...
        # graph_b entity references episode "missing" which doesn't exist in graph_b,
        # so create the episode first to satisfy FK constraint
        _episode(graph_b, "missing", "# Same\nContent")
        ...
        assert (tmp_path / "graphs" / "a" / "library.db").exists()
        assert (tmp_path / "graphs" / "b" / "library.db").exists()
        assert graph_a.blobs_dir != graph_b.blobs_dir
        # V1.5: version snapshots are stored in content/versions/ not snapshots/
        assert len(list((tmp_path / "graphs" / "a" / "content" / "versions").rglob("*.md"))) >= 1
        assert len(list((tmp_path / "graphs" / "b" / "content" / "versions").rglob("*.md"))) >= 1
    finally:
        graph_a.close()
        graph_b.close()
...
def test_concept_version_rules_same_episode_dedup_cross_episode_append(tmp_path):
    ...
    try:
        ...
        # V1.5 dedup: same episode+family keeps first absolute_id
        assert versions[0]["absolute_id"] == "conver_1"
...
def test_remember_chunks_share_one_document_version(tmp_path):
    ...
    try:
        ...
        assert len(docs) == 1
        doc = docs[0]
        assert doc["source_mode"] == "managed"
        # V1.5: managed_path is set but the actual content is in content/versions/
        assert doc["managed_path"].startswith("content/")

        conn = store._conn()
        # V1.5: one document, one active document_version
        assert conn.execute("SELECT COUNT(*) FROM document_versions WHERE status = 'active'").fetchone()[0] == 1
        # Two episodes created
        assert conn.execute("SELECT COUNT(*) FROM episodes WHERE status = 'active'").fetchone()[0] == 2

        # V1.5: verify the document_version stores char_count and line_count
        ver = conn.execute(
            "SELECT char_count, line_count FROM document_versions WHERE status = 'active'"
        ).fetchone()
        assert ver["char_count"] == len(full_text)
        assert ver["line_count"] == len(full_text.splitlines())

        # Verify content through get_document_content
        content = store.get_document_content(doc["document_version_id"], offset=0, limit=100000)
        assert content["content"] == full_text

        # Episode detail
        ep_detail = store.get_episode_content_detail("epver_chunk_2")
        assert ep_detail["source_text"] == "Bob uses SQLite."

        # V1.5: HAS_EPISODE edges visible via graph_edges view
        edges = conn.execute(
            "SELECT edge_type FROM graph_edges WHERE edge_type = 'HAS_EPISODE'"
        ).fetchall()
        assert len(edges) == 2
    finally:
        store.close()
...
def test_cache_lookup_is_scoped_by_document_path(tmp_path):
    ...
    try:
        ...
        store.save_episode(ep_b, text=ep_b.content, document_path=str(path_b), doc_hash="same_chunk_b")
        store.save_episode(ep_a, text=ep_a.content, document_path=str(path_a), doc_hash="same_chunk_a")

        found = store.find_cache_by_doc_hash("same_chunk_b", document_path=str(path_b))
...
def test_mentions_edge_records_sentence_evidence(tmp_path):
    ...
    try:
        ...
        # V1.5: entity_mentions table instead of concept_edge
        rows = store._conn().execute(
            """
            SELECT entity_family_id, surface_text, start_offset, end_offset
            FROM entity_mentions
            WHERE episode_id = 'epver_evidence'
            ORDER BY entity_family_id
            """
        ).fetchall()

        mentions = {row["entity_family_id"]: row for row in rows}
        assert "confam_alice" in mentions
        assert mentions["confam_alice"]["surface_text"] != ""
        assert mentions["confam_alice"]["start_offset"] >= 100

        assert "confam_deep_dream" in mentions
        assert mentions["confam_deep_dream"]["surface_text"] != ""

        atlas = Entity("conver_atlas", "confam_atlas", "Project Atlas", "A project", now, now, "epver_evidence", "Doc.md")
        store.save_entity(atlas)
        store.save_episode_mentions("epver_evidence", [atlas.absolute_id])
        row = store._conn().execute(
            """
            SELECT surface_text, start_offset
            FROM entity_mentions
            WHERE entity_family_id = 'confam_atlas'
            ORDER BY created_at DESC LIMIT 1
            """
        ).fetchone()
        assert row is not None
        assert row["surface_text"] != ""
    finally:
        store.close()
...
def test_vault_index_parses_obsidian_links_and_heading_episodes(tmp_path):
    ...
    (vault / "C.txt").write_text("Plain text document", encoding="utf-8")
    store = _store(tmp_path, "g")
    try:
        ...
        assert len(docs) == 3
        assert all(d["source_mode"] == "external" for d in docs)
        # V1.5: vault files get absolute_path set during indexing
        assert all(d.get("absolute_path") or d.get("managed_path") for d in docs)
        ...
        # at minimum the documents and episodes should exist
        assert len(docs) == 3
    finally:
        store.close()
...
def test_document_graph_returns_document_episode_concept_subgraph(tmp_path):
    store = _store(tmp_path, "g")
    try:
        _episode(store, "epver_doc_a", "# A\nAlice uses SQLite with Deep-Dream", source="DocA.md")
        _episode(store, "epver_doc_b", "# B\nBob also uses SQLite", source="DocB.md")
        now = datetime.now()
        ...
        # Concepts collected from edges
        family_ids = {c["family_id"] for c in graph["concepts"]}
        assert "confam_alice" in family_ids or "confam_sqlite" in family_ids or len(graph["concepts"]) > 0
    finally:
        store.close()
...
def test_document_graph_outline_and_chunk_are_progressive(tmp_path):
    store = _store(tmp_path, "g")
    try:
        _episode(store, "epver_prog_a", "# A\nAlice uses SQLite with Deep-Dream", source="ProgA.md")
        _episode(store, "epver_prog_b", "# B\nBob also uses SQLite", source="ProgB.md")
        now = datetime.now()
