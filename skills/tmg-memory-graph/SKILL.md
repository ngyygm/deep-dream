...
| `async` | 否 | bool | 默认 `true`——异步执行，立即返回 `task_id`；`false` 同步等待 |

### 异步模式（默认）

Remember 默认为异步模式：请求立即返回 HTTP 202 + `task_id`，后台流水线并行处理。多个 remember 请求可以并发执行（记忆缓存更新阶段串行保序，抽取/处理阶段并行加速）。

```bash
# 提交
curl -X POST http://127.0.0.1:16200/api/remember -H 'Content-Type: application/json' \
  -d '{"text": "...", "source_name": "demo"}'
# → {"success": true, "data": {"task_id": "abc123", "status": "queued", ...}}

# 轮询状态
curl http://127.0.0.1:16200/api/remember/status/abc123
# → {"success": true, "data": {"status": "completed", "result": {...}}}

# 查看队列
curl http://127.0.0.1:16200/api/remember/queue
```

如果需要同步等待，传 `"async": false`，请求将阻塞直到处理完成。
...
- 按时间顺序整理成叙事文本，带上时间戳。
- 一次性 POST 到 TMG `/api/remember`（不要逐句发送）。默认异步，拿到 task_id 后继续其他工作，稍后可检查状态。
