...
```bash
python -m core.server.api --config service_config.json --host 0.0.0.0 --port 16200
```

- `--host 0.0.0.0` 监听所有网卡（service_config.json 中也配置了 `"host": "0.0.0.0"`）
- 端口 16200
- 后台启动示例：`nohup python -m core.server.api --config service_config.json --host 0.0.0.0 --port 16200 > /tmp/dd_server.log 2>&1 &`
...
命令之间可以组合。例如从时间旅行到搜索、从搜索到遍历、从遍历到溯源。
...
从文本中提取的一切内容——实体、关系、Episode——都是概念（Concept）。概念是知识图谱的最小语义单元。

- 人类记忆中"一个事物"、"两件事物的关联"、"一段经历"没有硬边界，它们都是记忆，可以被回忆、修正、连接和遗忘
- 三种角色是同一原语的不同面：entity（是什么）、relation（怎么关联）、observation（经历了什么）。角色不是类型
- 对比传统知识图谱：关系不再是边的属性，而是一等公民概念——"Python被用于AI"可以被独立搜索、讨论、修正和版本化
- 所有面向"概念"的逻辑（embedding、搜索、版本迭代）应对三种角色一视同仁

### 第二条：提取最大化

概念抽取的核心目标是**越多越好**。未来使用时才能找得更准确、探索得更全面。

- 人类记忆捕获一切。今天看似琐碎的内容，可能成为未来洞察的关键
- 漏提取（假阴性）比多提取（假阳性）危害大得多：多的可过滤，漏的永远找不回来
- 不确定是否该提取时，提取。质量通过置信度评分和后续过滤保障，不在提取阶段收紧
- 同时适用于实体和关系——孤立实体和缺失关系都是知识图谱的空洞

### 第三条：实体 = 记忆锚点

实体提取不只是传统 NER（人名/地名/组织名），而应提取**人类回忆这段文本时会第一时间想到的一切**。

- 传统类别：具名实体、核心概念、物品场所、事件制度、技术术语、抽象概念
- **记忆锚点**——人类回忆时最先浮现的内容：
  - 一句话：金句、口头禅、标志性台词（如"好了歌"）
  - 网址链接：URL、文件路径、代码引用
  - 核心思想：文本的主旨论点、哲学观点（如"色空观"）
  - 特殊事件：标志性场景、转折点（如"黛玉葬花"）
  - 疑惑点：令人困惑的内容、未解答的问题
  - 新奇内容：从未见过的独特概念
  - 错误：文本中提及的错误信息、异常现象
- Prompt 设计原则：**不硬编码类型枚举**，用通用认知语言描述——"提取人类回忆时会想到的一切"，上述列表供 prompt 设计者参考
- 实体命名：使用文本中的原始名称，同一实体的不同称谓只提取一次（优先标准名/本名）

### 第四条：关系 = 自然语言关联

关系提取不是把谓语分类，而是捕获**两个概念之间人类会自然联想到的一切联系**。

- 人类描述关系用的是故事而非谓词分类："A叫了B"、"A把这个给了B"、"A先做了X然后B做了Y"
- 传统类别：归属/因果/使用/对比/层级/举例/属性
- **扩展类别**——一切有关联的都应连接：
  - 函数调用：`A()`调用了`B()`
  - 人物关系：不只是"属于"，而是"A是B的导师"、"A背叛了B"
  - 交互行为："A与B争论"、"A赠B以物"
  - 行为链："A做了X后B做了Y"
  - 时序关系："A发生在B之前"
  - 空间关系："A在B的旁边"
- 关系内容必须具体到人类一句话能说清。泛泛描述（"有关联""存在关系"）无效
- Orphan recovery 是必须步骤——没有关系的实体是图谱的孤岛

### 第五条：Episode = 观测切片

Episode 是特殊的观测概念（`:Episode :Concept`），是所有提取概念的**溯源锚点**。

- 人类记忆不整本存储一本书，而是存储离散的"经历片段"，每个片段有自己的上下文和情绪
- Episode 结构：原始文本片段 + 状态记忆（当前摘要 + 自我思考）
- **MENTIONS 合约（不可违背）**：所有从该 Episode 提取的实体和关系，必须通过 MENTIONS 边链接回该 Episode
  ```
  (Episode) -[:MENTIONS]-> (Entity)
  (Episode) -[:MENTIONS]-> (Relation)
  ```
- 不存在没有来源的概念，也不存在没有子概念的 Episode

### 第六条：窗口内合一

同一个 Episode（窗口）内，同一概念只保留一个代表。

- 一段话中三次提到"北京"，你不会形成三个独立记忆，而是形成一个可能更丰富的记忆
- LLM 可能把同一实体的不同叫法当成多个实体一次输出，需要窗口内对齐合并
- 合并策略：标准名/本名为主名称，括号标注别名或不同称呼，或在 content 中说明
- 结果：每 Episode 每概念只有一个版本（一个 absolute_id）、一条 MENTIONS

### 第七条：跨窗口同一性判定

不同 Episode 提到的概念，通过多信号判定是否为同一概念。

- 判定信号（优先级从高到低）：
  1. **类型**（最强负信号）：类型不同 → 绝不合并
  2. **名称**（最强正信号）：精确匹配 > 核心名匹配 > 子串匹配 > 语义相似
  3. **内容**：名称模糊时，对比 content 是否描述同一对象
  4. **状态记忆上下文**：Episode 的状态记忆提供领域和语境信号
  5. **图谱上下文**（角色指纹）：连接的其他概念高度重合 → 可能同一
- 三值判断：`same` / `different` / `uncertain`，`uncertain` 当 `different`
- **宁可漏合并也不误合并**：漏合并可以后续修正，误合并会污染版本历史
- "A提及B" ≠ "A和B是同一对象"，content-mention dedup 保持禁用

### 第八条：版本即时间线

每次跨 Episode 提及一个概念，都创建一个新版本。**版本数 = Episode 提及数**。

- 认知动机：每次重新遇到一个概念，即使没有新信息，"我又见到了它"本身就是一次记忆事件
- 硬规则：每次跨 Episode 对齐都产生新 `absolute_id`，即使内容与上一版本完全相同
- 版本与内容更新是两件独立的事：
  - 有新内容（增量信息）→ 合并后的新版本
  - 无新内容（子集或完全相同）→ 复用旧内容但仍创建新版本（新 absolute_id）
  - 纠正错误 → 新版本替换被纠正的表述
- Git 类比：`family_id` = 分支名（稳定逻辑 ID），`absolute_id` = commit hash（版本快照），`valid_at`/`invalid_at` = 时间窗口
- **例外**：同一 Episode 内去重后的概念只创建一个版本（见第六条）

### 第九条：内容合并 = 增量快进

当概念内容需要更新时，遵循 fast-forward 策略：在旧理解上做最小插入，不推倒重来。

- 人类学习新知识是在已有理解上添加，不是从零重建
- 合并层级：
  1. 新信息是旧内容子集 → 复用旧版本原文（但仍创建新 absolute_id，见第八条）
  2. 新信息有实质增量 → 最小插入，不改变已有表述
  3. 新版本修正事实错误 → 替换被纠正的部分
  4. **不丢信息**
- Content 使用 Markdown sections 结构 + ContentPatch 记录增量变更
- 每个版本可以选择查看全量内容变化，或只看有实质更新的变化

### 第十条：回忆 = 人类式检索

Find 应该像人类回忆一样工作——先找到概念，再追溯它的"记忆"。

- 人类回忆是从模糊印象出发，通过联想逐步定位到具体记忆
- 搜索范式：精确匹配+ 向量（语义接近）+ 图谱扩展（结构关联）+ RRF 融合
- **版本时间线**（用户可选择）：
  - 全量版本：所有版本，包括"又被提及了但内容没变"的（版本数 = Episode 提及数）
  - 仅内容变更版本：只有 name 或 content 有实质更新的版本
- **Episode 追溯**：从任何概念出发，通过 MENTIONS 追溯到所有提及它的 Episode，查看原始文本和当时的状态记忆

### 工程约束

- **Prompt 设计**：每个 prompt 控制在 20 行核心指令以内，不硬编码类型枚举，不针对测试数据写特殊规则。
- **一个 LLM 调用只做一件事**：输出格式简单（JSON 数组/对象/布尔值），对输出做鲁棒解析
- **测试数据多样化**：覆盖英文、HTML、代码、博客、日志，不只用中文古典文学。核心指标：重定向数 < 10、独立实体数越多越好、0 个误合并是底线
- **优化先借鉴开源**：参考 OpenIE、REBEL、Entity Disambiguation、Entity Linking 方法，不自己造轮子
