...
def test_load_config_normalizes_new_grouped_schema(tmp_path: Path):
    ...
    assert cfg["pipeline"]["remember"]["mode"] == "multi_step"
    assert cfg["pipeline"]["remember"]["anchor_recall_rounds"] == 1
    assert cfg["pipeline"]["remember"]["named_entity_recall_rounds"] == 1
    assert cfg["pipeline"]["remember"]["entity_content_batch_size"] == 6
    assert cfg["pipeline"]["remember"]["entity_write_batch_size"] == 6
    assert cfg["pipeline"]["remember"]["relation_hint_rounds"] == 1
    assert cfg["pipeline"]["remember"]["relation_expand_rounds"] == 1
    assert cfg["pipeline"]["remember"]["pre_alignment_validation_retries"] == 2
    assert cfg["pipeline"]["remember"]["alignment_policy"] == "conservative"
