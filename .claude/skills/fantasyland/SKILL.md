...
# 关联探索
"请使用 deep-dream 技能，访问记忆库（graph_id: "{graph_id}"），
执行 concepts/search 和 traverse，返回相关概念、路径和可用伏笔。"
```
...
5. 用子 Agent 执行 `concept search / traverse` 发现隐藏关联
...
5. 用子 Agent 执行 `concept search / traverse`
