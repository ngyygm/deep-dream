...
_HINT_PATTERNS = [
    (lambda m: ("not found" in m or "未找到" in m) and ("entity" in m or "实体" in m),
     "Hint: use GET /api/v1/concepts/search?query=X to find the correct family_id."),
    (lambda m: ("not found" in m or "未找到" in m) and ("relation" in m or "关系" in m),
     "Hint: use GET /api/v1/concepts/{family_id}/neighbors to find relations of an entity."),
    (lambda m: ("not found" in m or "未找到" in m) and ("episode" in m or "记忆" in m),
     "Hint: use GET /api/v1/documents to list documents, then /api/v1/concepts/{family_id}/provenance for mentions."),
    (lambda m: "not found" in m and "task" in m,
     "Hint: use GET /api/v1/remember/tasks to list all tasks with valid IDs."),
    ...
    (lambda m: "already exists" in m,
     "Hint: use GET /api/v1/concepts/search?query={name} to check if it already exists."),
    ...
    (lambda m: "合并" in m and "差异" in m,
     "Hint: merge was rejected because source and target names differ too much. Pass skip_name_check: true in the request body to force merge."),
    (lambda m: "合并自身" in m or ("merge" in m and "itself" in m),
     "Hint: the target entity cannot also be a source entity. Remove it from source_family_ids."),
    (lambda m: "自引用关系" in m or ("self-referenc" in m),
     "Hint: entity1 and entity2 must be different. A relation cannot connect an entity to itself."),
