...
class _PipelineMixin:
    ...
    def remember_text(self, text: str, doc_name: str = "", verbose: bool = False,
            ...
            try:
                ...
                if _set_publish_state:
                    _set_publish_state(
                        ...
                        complete_windows=len(_done_indices),
                        missing_windows=[i for i in range(total_chunks) if i not in _done_indices],
                    )
        ...
        if _set_publish_state:
            # Recompute publication from the document's persisted active
            # windows, rather than assuming that a targeted repair implies
            # every other window already exists.  A new document repaired at
            # W0 must remain hidden until W1..Wn are actually present.
            _publish_document_id = _atomic_document_id
            ...
            _active_window_indices.difference_update(_failed_window_indices)
            ...
            if not _active_window_indices:
                _active_window_indices = set(_successful_window_indices)
