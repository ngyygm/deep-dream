...
# 单轮收敛扫描的判定对数上限（同名组优先排在前；实测 16 docs 已有 167 个
# 子串别名候选，45 docs 满 scope 需更大预算——250 对 ≈ 8-10 min 串行收尾）
_SWEEP_MAX_PAIRS = 250

DUPES_KEY = "__duplicate_groups__"


def align_v2_enabled() -> bool:
    return os.environ.get("DD_ALIGN_V2") == "1"
...
def final_convergence_flush(processor, verbose: bool = False) -> Dict[str, int]:
    ...
    try:
        for _round in range(3):  # 合并可能级联出新别名对，最多 3 轮
            ...
            for pa, pb in alias_pairs:
                judge_jobs.append((pa, pb, "substring"))
            judge_jobs = judge_jobs[:_SWEEP_MAX_PAIRS]
            ...
            for pa, pb in alias_pairs[:_SWEEP_MAX_PAIRS]:
