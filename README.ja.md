...
3. **Agent が行うこと**  
   - サービスが未起動なら: リポジトリ clone → `service_config.json` 設定 → `python service_api.py` 起動 → `GET /api/health` で確認。  
   - 記憶: `POST /api/remember` に JSON フィールド `text` または `text_b64`（まとまった内容をバッチで送信し、一文ずつの呼び出しは避ける）。  
...
POST JSON 本文（form も可）。`text` と `text_b64` のどちらかが必須。まとまった内容をバッチで送信し、一文ずつの呼び出しは避けてください。

| パラメータ | 必須 | 説明 |
|------------|------|------|
| `text` | `text_b64` とどちらか | 自然言語テキスト |
...
- **原子 API:** エンティティ検索（`/api/find/entities/search` 等）、関係、記憶キャッシュ、統計（`/api/find/stats`）、条件に基づく一括取得（`POST /api/find/candidates`）。
