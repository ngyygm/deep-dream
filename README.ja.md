...
3. **Agent が行うこと**  
   ...
   - 記憶: `POST /api/remember` に JSON `text`（まとまった内容をバッチで送信、一文ずつの呼び出しは避ける）。  
...
JSON body のみ、`text` が必須。まとまった内容をバッチで送信し、一文ずつの呼び出しは避けてください。

| フィールド | 必須 | 説明 |
|------------|------|------|
| `text` | はい | 自然言語テキスト |
| `source_name` | いいえ | ソースラベル |
| `event_time` | いいえ | ISO 8601 — イベントが実際に発生した時刻（省略時は処理時間） |
| `load_cache_memory` | いいえ | 最新の記憶キャッシュチェーンから継続するか |

サービスは全文を `storage_path/originals/` に保存し、`original_path` を返します。内部で切片化・記憶キャッシュ更新・エンティティ/関係抽出・グラフ整合・バージョン書き込みを実行。
